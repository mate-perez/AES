...
# Función para calcular el error LOOCV
...
# Errores para cada modelos
errors <- c(
  loocv_error(modelo1, data),
  loocv_error(modelo2, data),
  loocv_error(modelo3, data),
  loocv_error(modelo4, data)
)
...
Para $\lambda = \infty$ y m creciente da como resultado curvas más suaves y restringidas cada vez. Para $\lambda = 0$, la curva puede ser más flexible respetando la restricción de suavidad de la tercera derivada.
...
ggplot(mapping = aes(X_values, Y_values)) +
  geom_line(col = "#0073e6", lwd = 1.5)
...
spline_summary <- spline_fit %>% extract_fit_engine() %>% summary()
spline_summary
...
ggplot(boston_data, aes(x = dis, y = nox)) +
  ...
  geom_line(aes(y = predicts__10), color = "#FF6F19", size = 1) +
  geom_line(aes(y = predicts__14), color = "#FFDB19", size = 1) +
  geom_line(aes(y = predicts__18), color = "#1AABFF", size = 1) 
...
```{r}
rmse_linear <- broom::augment(fit,
                new_data = college_train) %>% rmse(Outstate, .pred)
...
rmse_gam <- broom::augment(gam_fit,
                new_data = college_train) %>% rmse(Outstate, .pred)
